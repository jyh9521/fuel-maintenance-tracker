...
| 変数名 | 説明 | デフォルト値 | 必須 |
|--------|------|--------------|------|
| `DATABASE_URL` | SQLite データベースファイルのパス | `file:/app/prisma/db/dev.db` | いいえ |
...
> [!WARNING]
> **自動マイグレーションについて**: コンテナ起動時に、データベースのスキーマ更新が自動的に確認・適用されます。新しいバージョンに破壊的なスキーマ変更（フィールドの削除など）が含まれる場合、**システムは起動時に自動的にこれらの変更を適用します**。定期的にデータベースファイルをバックアップすることを強く推奨します。
